...
<?php
        ...
        $db_com_name = mysqli_query($conn, "SELECT com_ip FROM computer_select where com_name = '$com_name'") or die(mysql_error()); 
        while($row = mysqli_fetch_array($db_com_name)) 
        { 
            $com_ip = $row['com_ip']; 
        }

        $data = mysqli_query($conn, "SELECT * FROM computer_log where com_ip = '$com_ip' ORDER BY `computer_log`.`datetime` DESC")  or die(mysql_error()); 
